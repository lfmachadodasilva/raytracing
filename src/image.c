...
typedef struct
{
	float *max;     /* maximos R's, G's e B's */
	float *min;	   /* minimos R's, G's e B's */
...
static int getlong(FILE *input, long int *longint)
{
	...
	templongint = ((long int)(temp[COLOR])<<24) | ((long int)(temp[2])<<16) 
...
static int putlong(FILE *output, long int longint)
{
	...
	temp[COLOR] = (unsigned char)(longint >> 24) & 0xff;
...
static int getdword(FILE *input, unsigned long int *dword)
{
	...
	tempdword = ((unsigned long int)(temp[COLOR])<<24) | ((unsigned long int)(temp[2])<<16) 
...
static int putdword(FILE *output, unsigned long int dword)
{
	...
	temp[COLOR] = (unsigned char) ((dword >> 24) & 0xff);
...
Image * imgGrey(Image * image)
{
	...
	for (y=0;y<h;y++){
		for (x=0;x<w;x++) {
			imgGetPixel3fv(image,x,y,rgb);
			grey[RED]=0.2126f*rgb[RED]+0.7152f*rgb[GREEN]+0.0722f*rgb[BLUE];
			grey[GREEN]=grey[0];
			grey[BLUE]=grey[0];
...
Image * imgResize(Image * img0, int w1, int h1)
{
	...
	for (y1=0;y1<h1;y1++)
		for (x1=0;x1<w1;x1++)
		{
			...
			y0=ROUND(h0*y1/h1);
			imageGetPixel(img0,x0,y0, color);
...
//	int w1,h1;
//	int x,y;
//	float rgb[COLOR],black[COLOR]={0.f,0.f,0.f};
//
...
void imgGetWidth(Image *image, int *out_int)
{
	(*out_int) = image->width;
}

void imgGetHeight(Image *image, int *out_int)
{
	(*out_int) = image->height;
}

void imgGetRGBData(Image * image, float *out_color)
{
	colorCopy( image->buf, out_color );
}

void imgSetPixel3fv(Image * image, int x, int y, float * color)
{
	int pos = (y*image->width*3) + (x*3);
	image->buf[pos  ] = color[RED  ];
	image->buf[pos+1] = color[GREEN];
	image->buf[pos+2] = color[BLUE ];
}
...
void imgSetPixel3ubv( Image * image, int x, int y, unsigned char * color )
{
	...
	image->buf[pos  ] = (float)(color[RED  ]/255.);
	image->buf[pos+1] = (float)(color[GREEN]/255.);
	image->buf[pos+2] = (float)(color[BLUE ]/255.);
}
...
void imgGetPixel3ubv( Image * image, int x, int y, unsigned char *color )
{
	...
	color[RED  ] = (unsigned char)(r < 256.0f) ? r : 255.0f;
	color[GREEN] = (unsigned char)(g < 256.0f) ? g : 255.0f;
	color[BLUE ] = (unsigned char)(b < 256.0f) ? b : 255.0f;
}

void imageSetPixel(Image *image, int x, int y, float *color)
{
	int pos = (y*image->width*3) + (x*3);
...
void imageGetPixel(Image *image, int x, int y, float *out_color)
{
	int pos = (y*image->width*3) + (x*3);

	out_color[RED  ] = (image->buf[pos]);
	out_color[GREEN] = (image->buf[pos+1]);
	out_color[BLUE ] = (image->buf[pos+2]);
}
...
int imgWriteBMP(char *filename, Image * bmp)
{
	...
	for (k=0; k<bmp->height;k++)
	{
		...
		for (i=0; i<bmp->width; i++) {
			...
			filedata[l-1] = (unsigned char)(b < 256) ? b : 255.0f;
			filedata[l  ] = (unsigned char)(g < 256) ? g : 255.0f;
			filedata[l+1] = (unsigned char)(r < 256) ? r : 255.0f;
			j+=3; l+=3;
		}
...
//	Image * img1 = imgCreate(w,h);
//
//	float rgb00[COLOR], rgb01[COLOR], rgb02[COLOR];
//	float rgb10[COLOR], rgb11[COLOR], rgb12[COLOR];
//	float rgb20[COLOR], rgb21[COLOR], rgb22[COLOR];
//	float rgb[COLOR];
//
//	int Gauss[COLOR][COLOR];
//
//	int x, y;
//
//	/* Filtro Gaussiano */
